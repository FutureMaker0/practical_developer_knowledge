...
### 클라우드 서비스를 사용할 때의 공유 책임 모델
  <img width="550" src="https://github.com/FutureMaker0/designPattern_singleton/assets/120623320/f0484b56-d23e-4648-a56b-67105284ca8e"></br>
  <출저 - aws.amazon.com>
  - 공유 책임 모델에 따라 클라이언트가 책임지는 보안 영역이 변화한다.
  - EC2는 IaaS이므로, 인프라 영역의 보안까지는 AWS에서 관리한다. 그러나 상위 플랫폼과 애플리케이션 영역 보안은 클라이언트가 직접 관리해야 한다.(애플리케이션 시큐어 코딩 등)

### 다양한 보안 서비스
  - 방화벽, 웹 방화벽, 침입 방지 시스템(IPS) 등의 서비스도 존재한다.
  - EC2를 사용하면, 기본적으로 방화벽 서비스인 Security Group이 서비스된다.
  - 방화벽은 IP주소와 Port를 확인하여 트래픽을 허용 또는 차단하는 서비스이다. 디폴트 값으로 모든 트래픽이 막혀 있으므로 사용할 IP와 Port를 설정해주어야 한다.
  - 웹 방화벽은 애플리케이션의 주요 취약점을 공격하려는 시도를 탐지하여 차단한다. SQL 인젝션, 크로스 사이트 스크립트(XSS)등 주요 취약점을 차단한다. 필터 설정을 통해 특정 공격
    패킷을 차단하므로 개발자 필수 보안 서비스라 할 수 있다.(애플리케이션 수준 보안 서비스)
  - IPS는 네트워크 수준의 보안 서비스이다. DDoS, SYN flooding 등 네트워크 수준 공격 탐지 및 차단.

### 클라우드에서 지원하는 데이터 암호화 서비스
  - 애플리케이션에서 비밀번호, 개인정보 같은 정보를 취급한다면 데이터 암호화는 선택이 아닌 필수이다.
  - 데이터 암호화를 위해 코드를 통한 시큐어 코딩이 가능하지만, 클라우드 서비스를 사용하면 이조차도 제공되는 서비스를 통해 암호화가 가능하다.
  - AWS Encryption SDK
    - 클라우드 스토리지와 데이터베이스에 저장된 데이터 암복호화 지원.
    - C, java, python 등 다양한 언어를 지원한다.
  - 클라우드 서비스를 통한 HTTPS 간편설정 및 인증서 자동 갱신.

### 외부 접근 통제를 위한 인트라넷 서비스
  - 외부 네트워크와 직접 통신이 필요하지 않은 서버는 일반적으로 인트라넷으로 구성하여 구분한다.
  - 웹 서버, 메일 서버, DNS 서버 등은 외부망과 연결하되, 업무용 서버/데이터베이스는 인트라넷으로 구성하여 아래와 같이 보안 체계를 구축할 수 있다.
  - 인터넷 - 방화벽 - 웹서버/메일서버/DNS서버 - 방화벽 - 업무용서버/DB서버
